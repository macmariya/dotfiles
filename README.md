...
このリポジトリは個人の環境に合わせた設定を含んでいます。**必ず fork してから使用してください。** 直接クローンして使うと、自分のカスタマイズを upstream に push できず、設定の追跡・管理が困難になります。
 
```bash
# 1. GitHub 上でこのリポジトリを fork
# 2. fork したリポジトリをクローン
...
- **Google Chrome 自動セットアップ** -- Homebrew Cask でインストールし、`defaultbrowser` CLI でデフォルトブラウザに設定
