...
# 5. tmux プラグインをインストール（tmux 起動後）
# prefix + I
```

`bootstrap.sh` は以下の 9 フェーズを順に実行する。すべて冪等で、再実行しても安全。
...
| 7 | シークレット設定チェック（Keychain） |
| 8 | macOS システム設定（対話式、スキップ可） |
...
```
dotfiles/
├── bootstrap.sh              # 初回セットアップ（9フェーズ、冪等）
